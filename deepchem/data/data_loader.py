...
def mol2graph(smi):
    ...
    temp_A = Chem.rdmolops.GetAdjacencyMatrix(mol).astype(np.float32, copy=False)[:MAX_LEN, :MAX_LEN]
    num_atom = temp_A.shape[0]
    A[:num_atom, :num_atom] = temp_A + np.eye(temp_A.shape[0], dtype=np.float32)

    de = A.sum(axis=1)[:, np.newaxis]
    de[de <= 0] = 1
    A = A / de
...
def preprocess_df(smiles, num_worker):
    ...
    mols = sorted(mols, key=lambda x: x[2])
    return mols
